...
\begin{document}
	...
	\begin{center}
		...
		\midrule 
		Source & A system that wants to fetch or modify the data\\
		\\
		Stimulus & Requests to access and overwrite the original data\\
		\\
		Artifact & A database, container or some other repository that \\
		& holds the data\\
		\\
		Environment & Runtime, in service. And also, maybe parts of \\
		& the whole system are out of the service, \\
		& but the database is still on.\\
		\\
		Response & Make sure the data would be changed in a right way. \\
		& May do the following things:\\
		& 1. Check whether the request is valid. \\
		& e.g. authenticated and authorized.\\
		& 2. Check whether the data is valid\\
		& e.g. if there is a software bug or hardware bug.\\
		& that make it wrong or meaningless.\\
		\bottomrule
		\end{tabular}
		\caption{Data Integrity General Scenario}
		\end{table}
	\end{center}
	...
	\subsection{Views}
		...
		\subsubsection{Decomposition View} 
			\paragraph{Section 1: The Primary Presentation}
			\begin{center}
			% \includegraphics[scale=0.3]{https://i.loli.net/2019/11/03/FhGD3rlzqpYj6s1.png}
			\end{center}
			\paragraph{Section 2: The Element Catalog}
			...
			\paragraph{Section 3: Context Diagram}
			\begin{center}
			% \includegraphics[scale=0.3]{https://i.loli.net/2019/11/03/AiSygsqvFdRPCMQ.png}
			\end{center}
			\paragraph{Section 4: Variability Guide}
