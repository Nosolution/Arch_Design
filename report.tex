		...
		\subsubsection{Iteration 2}
			...
			\begin{center}
				\begin{tabular}{|c|c|}
					...
					\hline
					\multirow{1}*{Service recovery} & Recovery from the default\\
					\hline
					\multirow{1}*{Service Registry} & Service find\\
					\hline
				\end{tabular}
			\end{center}
...
In a nutshell, it is useful to follow these steps. In the last phase, when I put all the parts together, I find it is natural to reach the final result with our refinement, which is not such an obvious result from the first glance of the requirement document.
	...
	\subsection{Statement of Guozhao Huang}
	\paragraph{1: ADD experience}
	As a continuous progress of refinement in architecture design, ADD expresses its great advantages. Clearly, ADD requires us to design according to ASRs. Each iteration we need to find a way to satisfy some ASRs, not allowed to desultorily imagine. Besides, this top-down design method constrains the split of elements. Consistency of ASRs forces the element to maintain its relation with others, which will contribute to the compactness of the system. \\
	But when applying ADD, I found it a little hard to handle the degree of decomposition. I sometimes fell into the hole of overspecification, blocking me from next iteration. So I felt that caution is necessary during iteration.
	\paragraph{2: Contribution}
	I participated in brainstorm of ASR specification, first iteration, health detection part of second iteration, half of the 4th iteration, handling the framework and writing beyond views part of the report and acted as a system sketch provider during iterations.\\
	I was also the main organizer of out team, responsible to organize meetings and handle the general direction of the workflow and work assignment. I was somehow like a regulator as I liked to review and confirm if we had followed the right step and left nothing unconsidered.
